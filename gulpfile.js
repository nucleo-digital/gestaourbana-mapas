...
gulp.task('svg', function() {
    return gulp.src(paths.svg.files)
        .pipe(svgmin())
...
gulp.task('default', ['sass', 'images', 'browserify', 'svg'], function() {
    ...
    gulp.watch(paths.svg.files, ['svg'])
    // Also when there is a change, display what file was changed, only showing the path after the 'sass folder'
    .on('change', function(evt) {
        console.log(
            '[watcher] File ' + evt.path.replace(/.*(?=svg)/,'') + ' was ' + evt.type + ', compiling...'
        );
    });
